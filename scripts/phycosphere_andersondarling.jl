...
function has_different_distribution(waiting_times, c, Δt)
    ...
    # compare sampled waiting times to reference pdf via Anderson-Darling test
    ADtest = OneSampleADTest(ustrip.(waiting_times), reference_pdf)
    # if p < 0.05 the two distributions are not the same
    return Float64(pvalue(ADtest) < 0.05)
end
...
function getadradius(grid, diffsensing, R)
    idx = findfirst(diffsensing .> 0.5)
    isnothing(idx) ? R : grid[idx]
end

function phycosphere_ad()
    ...
    sensing_data = produce_or_load(
        datadir("PoissonSampling", "AndersonDarling"), params;
    ...
    produce_or_load(
        ...
        S = [
            getadradius(grids[i], diffsensing[i,j], R[i])
