...
\sidecaptionvpos{figure}{t} % align caption to top in SCfigure
...
%The stakes are high for chemotactic encounters between bacteria and small phytoplankton.
Chemotactic encounters between bacteria and small phytoplankton are a high-stakes game.
}
...
Microscale interactions between bacteria and phytoplankton control nutrient cycling in the ocean. Chemotaxis can aid bacteria in navigating chemical gradients exuded by the phytoplankton, favoring their encounter by reducing the search times. Chemotactic searches requires bacteria to accurately detect noisy gradients, with the type of noise depending on the size of the phytoplankton cell.
Detection fails when gradients are too flat (large phytoplankton) or too spatially confined (small phytoplankton). We computed an upper bound on bacterial chemotactic advantage for different phytoplankton size classes and found that chemotaxis offers the largest reductions in search time for small phytoplankton. However, the advantage sensitively relies on the chemotactic strategy adopted by bacteria. Chemotactic encounters with small phytoplankton are thus a high-stakes game that may promote a diversity of size-dependent chemotactic strategies.
...
\correspondingauthor{\textsuperscript{1}To whom correspondence should be addressed. E-mail: jslomka@ethz.com}
...
%Chemotaxis, the ability to navigate chemical gradients, enables motile bacteria to increase encounter rates with phytoplankton cells, favoring microscale interactions.
Chemotaxis, the ability to navigate chemical gradients, enables motile bacteria to increase encounter rates with phytoplankton cells.
%However, it remains unclear how strongly the performance of chemotactic searches, which relies on the detection of noisy and spatially confined gradients around phytoplankton of different sizes, depends on the bacterial search strategies.
%However, it is unclear how the detection of noisy and spatially confined gradients around phytoplankton of different sizes depends on bacterial phenotypes, and how strongly the ensuing chemotactic behavior enhances encounters.
However, it is unclear how strongly the increase in encounter rates depends on the detection of noisy and spatially confined gradients around phytoplankton of different sizes.
...
Here, we compute an upper bound on the chemotactic index, a dimensionless number that measures the increase in encounter rate with a phytoplankton cell due to chemotaxis over random motility alone.
...
%We find that chemotaxis can strongly enhance the efficiency of searches for small phytoplankton, owing to the ballistic nature of encounters for those cells, but the chemotactic index, determined by the bacterial ability to resolve spatially confined gradients, is extremely sensitive to phytoplankton and bacterial phenotypes.
We find that chemotaxis can substantially speed up the search for small phytoplankton, owing to the ballistic nature of encounters for those cells. However, the chemotactic index is very sensitive to cells' phenotypes because spatially confined gradients around small phytoplankton limit the bacterial ability to resolve them.
%We find that chemotaxis can strongly enhance the efficiency of searches for small phytoplankton, owing to the ballistic nature of the encounter for those cells, but the chemotactic index is sensitive to phytoplankton leakage and bacterial motility, which limit the ability of bacteria to resolve spatially confined gradients.
%the bacterial ability to detect spatially confined gradients.
In productive waters, chemotaxis can decrease the search times of bacteria for micron-sized picophytoplankton up to ten-fold, from days to a few hours; but an increase in swimming speed or a slight reduction in phytoplankton size or leakage rate may render gradients undetectable, easily erasing this benefit.
%easily erase this benefit, rendering gradients undetectable.
...
%For larger phytoplankton, with radii above 10 micron, the decrease in search time is more moderate, from a month to a week, and depends gently on gradient detection abilities.
%For larger phytoplankton ($>\SI{10}{\micro\m}$), the decrease in search time is more moderate, from a month to a week, but, depending gently on gradient detection abilities, it is more robust to variations.
By contrast, for larger phytoplankton ($>\SI{10}{\micro\m}$), while the decrease in search time is more moderate, from a month to a week, it is more robust to variations because it depends gently on gradient detection abilities.
Thus, engaging chemotaxis in interactions with small phytoplankton is a high-stakes game for bacteria: the potential payoff is large, but it changes abruptly with bacterial ability to sense gradients, a scenario that may promote a diversity of chemotactic strategies.
...
Figures were realized with Makie.jl~\cite{} and Inkscape.
}
