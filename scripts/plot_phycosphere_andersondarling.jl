...
function plot_phycosphere_ad()
    datasets = collect_results(
        datadir("PoissonSampling", "AndersonDarling"),
    ...
    for df in eachrow(datasets)
        ...
        savefig(plotsdir("PoissonSampling", "AndersonDarling", savename("phycosphere", params, "svg")))
        savefig(plotsdir("PoissonSampling", "AndersonDarling", savename("phycosphere", params, "png")))
    end
end
