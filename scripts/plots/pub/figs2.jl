...
S(dynamic::Bool) = map(Iterators.product(radii, concentrations)) do (R, Cs)
    L = U*T
    f(r) = snr(U, Cdiff, ∇Cdiff, r, R, C0, Cs, T, Dc, a) - 1
...
ic(dynamic::Bool) = IC.(30.0u"μm", radii.+a, S(dynamic).+a)
...
for j in 2:-1:1
    ...
    contourf!(ax[j],
        ustrip.(radii), ustrip.(concentrations), log10.(ic(dynamic[j]));
