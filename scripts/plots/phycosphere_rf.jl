...
function plot_phycosphere_hein_byDc()
    f = jldopen(datadir("RF", "RC.jld2"))
    R, Cₛ = ustrip.(f["R"]), ustrip.(f["Cₛ"])
    close(f)

    alldatasets = unpack_dataframe(
        collect_results(datadir("RF"), rinclude=[r"phycosphere"], rexclude=[r"old"])
    )
    Ū = 50
    T̄ = 100
    Π̄ = 6
    ...
    save(plotsdir("RF", "phycosphere_varDc_T=$(T̄)_U=$(Ū)_Π=$(Π̄).svg"), fig)
end
...
function plot_phycosphere_hein_byU()
    ...
    save(plotsdir("RF", "phycosphere_varU_Dc=$(D̄)_T=$(T̄)_Π=$(Π̄).svg"), fig)
end
...
function plot_phycosphere_hein_byT()
    ...
    save(plotsdir("RF", "phycosphere_varT_Dc=$(D̄)_U=$(Ū)_Π=$(Π̄).svg"), fig)
end
